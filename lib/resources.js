...
import { isValidEmail, isValidPhone, isValidName, isValidDate, isValidOptionalText } from './util.js';
...
export const RESOURCES = {
  patients: {
    ...
    nums: [],
    // Patient text fields are rendered into the SPA and stored as-is, so they
    // get the same format gate the appointment fields do. Required names must be
    // non-blank; the optional fields only constrain shape/length. firstInvalidField
    // skips absent (undefined/null) values, so partial PUT updates still work.
    validate: {
      firstName: (v) => isValidName(v),
      lastName: (v) => isValidName(v),
      dob: (v) => isValidDate(v),
      phone: (v) => isValidPhone(v),
      email: (v) => v === '' || isValidEmail(v),
      sex: (v) => v === '' || ['F', 'M'].includes(v),
      address: (v) => isValidOptionalText(v, 200),
      allergies: (v) => isValidOptionalText(v, 200),
      medicalNotes: (v) => isValidOptionalText(v, 1000),
      insurance: (v) => isValidOptionalText(v, 120),
    },
  },
  treatments: {
    table: 'treatments',
